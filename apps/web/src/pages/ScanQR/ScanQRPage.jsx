...
import {
  Box, Paper, Typography, Button, Chip, CircularProgress, ButtonBase,
...
export function ScanQRPage() {
  ...
  const stopCamera = async () => {
    ...
    try { await codeReaderRef.current?.stop(); } catch { /* ignore */ }
    ...
    <Box sx={{ p: 2, maxWidth: 560, mx: 'auto' }}>
      <Paper sx={{ p: 3 }}>
        <Box sx={{ textAlign: 'center', mb: 2 }}>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>Escanear código QR</Typography>
        </Box>

        {!cameraOn && (
          <Box sx={{ textAlign: 'center', py: 3 }}>
            <QrCodeScannerIcon sx={{ fontSize: 64, color: 'primary.main', mb: 2 }} />
            <Typography color="text.secondary" sx={{ mb: 3 }}>Activa la cámara para escanear el QR de la invitación.</Typography>
            <Button variant="contained" size="large" onClick={startScanner}>Activar cámara</Button>
          </Box>
        )}

        {cameraOn && (
          <Box>
            <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', bgcolor: '#000', minHeight: 280 }}>
              <video ref={videoRef} muted playsInline style={{ width: '100%', minHeight: 280, objectFit: 'cover' }} />
              {frozenFrame && (
            ...
            <Box sx={{ mt: 2, textAlign: 'center' }}>
              {state === SCAN_STATES.SCANNING && <Chip label="Escaneando..." color="info" />}
            </Box>
          </Box>
        )}
      </Paper>
